...
#from swissknife.hilevel.ffnnkeras import get_raw_stream, scale_pc_inv
...
def pred_to_par_stream(y, fit_par_dict, s_f=44100):
    '''
    ...
    '''
    bin_size = fit_par_dict['bin_size']
    [alpha, beta, env] = fit_par_dict['par_reg_fcn_inv'](y.T)
...
def pred_par_to_song(y, fit_par_dict, s_f=30000):
    par_streams = pred_to_par_stream(y, fit_par_dict, s_f=s_f)
    syn_song = np.nan
...
def all_mot_decoded_pcwise(y, z, y_p, sess_data):
    ...
    try:
        pred_to_song_fcn = fp['pred_to_song_fcn']
    except KeyError:
        pred_to_song_fcn = lambda x: pred_to_song(x, fp['bin_size'])
    ...
    for i, m_id in tqdm(enumerate(mot_ids), total=mot_ids.size):
        ...
        mot_y_p = y_p[mot_idx]
        if fit_target == 'dyn':
            s_song, s_pars = pred_to_song_fcn(mot_y)
            n_song, n_pars = pred_to_song_fcn(mot_y_p)
