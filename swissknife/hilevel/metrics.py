...
from swissknife.hilevel.ffnn import get_raw_stream
...
def all_mot_decoded_pcwise(y, z, y_p, sess_data):
    ...
    logger.info('Getting streams and reconstructions of {} motifs'.format(mot_ids.size))
    all_decoded = []
...
def all_self_scores(one_pd, other_pd):
    ...
    for m_id, x, x_syn, x_raw in tqdm(zip(one_pd['m_id'].tolist(),
        ...
        cross_scores = np.array(
            list(map(lambda z: compare_spectra(x, z, n_perseg=128, db_cut=65)[0], other_pd['x'].tolist())))
