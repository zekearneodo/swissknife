...
def keras_ks_set(syl_df, bout_dict, fit_pars, units_list, flat_features=False, normalize_features=False, 
return_list=False):
    # all syl and units in unts_list are from the same bird and sess
    # (they share the kwd and kwik file)
    #syl_df = syl_df_in[:10]
    ...
    for i_syl, syl in tqdm(syl_df.iterrows(), total=syl_df.shape[0]):
        #print(i_syl)
        len_ms = int(syl['len_ms'])
        len_ap = int(len_ms * s_f_ap * 0.001)
        len_ni = int(len_ms * s_f * 0.001)
        
        offset_ms = - int(syl['t_pre_s'] * 1000) # offset within the sample taken in the bout_df
        logger.info('offset_ms {}'.format(offset_ms))
        offset_ap = int(offset_ms * s_f_ap * 0.001)
        offset_ni = int(offset_ms * s_f * 0.001)
        ...
        if target_type == 'spectrogram':
            pars_ms =  syl['rw_spec'][:, offset_ms: offset_ms + len_ms]
            pars_ms -= np.min(pars_ms)
            logger.info('pars_ms shape {}'.format(pars_ms.shape))
        ...
        ### make a less convoluted version of this, picking them up from the syl['spk_arr']
        ## s_v should be an np array [n_bins, n_units, n_trials]
        #ms_ap = int(1 * s_f_ap * 0.001) #samples in a milisecond for the ap band
        # make a ms_step version of the spk_arr
        # spk_arr comes in [n_unit, n_sample] array
        # how many ms to lookup
        history_ms = - fit_pars['bin_size'] * (fit_pars['history_bins'] + 1)
        offset_ap_ms = offset_ms + history_ms
        logger.debug('offset_ap_ms {}'.format(offset_ap_ms))
        spk_arr_ms = syl['ms_spk_arr'][:, offset_ap_ms: offset_ap_ms + len_ms + abs(history_ms)]
        logger.info('spk_ms shape {}'.format(spk_arr_ms.shape))     
        this_sv = bp.col_binned(spk_arr_ms, fit_pars['bin_size'])
        this_sv = np.expand_dims(this_sv, -1) # to make it a [n_unit, n_ms, 1] array for later
        ...
        #start in ms relative to motiff
        target_start_in_syl = np.arange(model_pars.shape[1]) * fit_pars['bin_size']
        target_start_in_mot = target_start_in_syl + offset_ni
        #logger.info('this_sv shape {}'.format(this_sv.shape))
        s_v.append(this_sv)
        target.append(model_pars)
        raw_target.append(np.stack([target_mot_id, target_syl_idx, target_start_in_syl, 
    ...
    if return_list:
        return fv_list, tv_list, rv_list
    else:
        return X.astype(np.float32), Y.astype(np.float32), X_t.astype(np.float32), Y_t.astype(np.float32), Z, Z_t
...
class ksData():
    def __init__(self, bird, sess, fit_pars, bout_df, bout_dict, clu_df, spk_df, derived_folder):
        logger.info('Init bird {0}, session {1}'.format(bird, sess))
        self.bird = bird
        self.sess = sess
        self.folder = derived_folder
        self.fp = fit_pars

        self.s_f = None
        self.all_syl = bout_df ## is this right?
        self.normalize_features = False # whether to normalize the feature vector sets
        self.thresh_factor = 4.5
    ...
    def get_s_f(self):
        self.s_f = self.bout_dict['s_f_ap_0']
        
    def load_syl(self):
        logger.debug('Loading syllable/bout data from {0}-{1} with file descr {2}'.format(self.bird, self.sess, self.fp['syl_file']))
        raise NotImplementedError
        
    def load_units(self):
        logger.debug('Loading units data for {0}-{1} with type {2}'.format(self.bird, self.sess, self.fp['unit_type']))
        logger.info('setting units list to all units picked in the bout dataframe')
        self.units_list = self.bout_df.loc[0, 'clu_id_arr']
        #raise NotImplementedError
        
    def init_set(self, test_trials=None):
        logger.info('Initializing data set for sess {}'.format(self.sess))
        #initialize a non-flat set
        self.X, self.Y, self.X_t, self.Y_t, self.Z, self.Z_t = keras_ks_set(self.all_syl, 
                                                                            self.bout_dict,
                                                                              self.fp,
                                                                              self.units_list,
                                                                              normalize_features = self.normalize_features,
    ...
    def gimme_set(self, test_trials=None):
        #flat_features = True if self.fp['network'] is 'ffnn' else False
        self.init_set()
        return self.X, self.Y, self.X_t, self.Y_t, self.Z, self.Z_t
    
    def gimme_set_for_pc(self, test_trials=None):
        #flat_features = True if self.fp['network'] is 'ffnn' else False
        fv_list, tv_list, rv_list = keras_ks_set(self.all_syl, self.bout_dict,
                                                                              self.fp,
                                                                              self.units_list,
                                                                              normalize_features = self.normalize_features,
                                                                              flat_features=None, return_list=True)
        all_len = np.array([x.shape[0] for x in fv_list])
        min_len = np.min(all_len)
        if abs(min_len - all_len.mean()) > all_len.mean()/8:
            raise ValueError('min len is {} and all lens are {}'.format(min_len, all_len))      

        stack = lambda x: np.stack([y[:min_len] for y in x], axis=1)
        f_arr, t_arr, r_arr = map(stack, [fv_list, tv_list, rv_list])
        return f_arr, t_arr, r_arr
